...
CREATE TRIGGER tr_reports_uat BEFORE UPDATE ON reports
...
SELECT
  ...
  bk.id AS booking_id, bk.status AS booking_status, bk.check_in_date,
  pg.id AS pg_id, pg.name AS pg_name, pg.address AS pg_address, pg.city AS pg_city,
  rm.room_code, rm.room_type, bd.bed_number, bd.price AS bed_price,
...
-- SELECT event_object_table, trigger_name FROM information_schema.triggers ORDER BY event_object_table;
